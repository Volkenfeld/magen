    ...
    const html = items
      ...
      <div class="item ${item.isAlert ? 'alert' : ''}" ${item.monitorColor ? `style="border-inline-start-color: ${escapeHtml(item.monitorColor)}"` : ''}>
        ...
        <div class="item-time">
          ...
          ${getCurrentLanguage() !== 'en' ? `<button class="item-translate-btn" title="Translate" data-text="${escapeHtml(item.title)}"><span style="font-size:10px;opacity:0.5">A&#1488;</span></button>` : ''}
        </div>
      </div>
    `
      )
    ...
    return `
      <div class="${itemClasses}" ${cluster.monitorColor ? `style="border-inline-start-color: ${escapeHtml(cluster.monitorColor)}"` : ''} data-cluster-id="${escapeHtml(cluster.id)}" data-news-id="${escapeHtml(cluster.primaryLink)}">
        ...
        <div class="cluster-meta">
          ...
          ${getCurrentLanguage() !== 'en' ? `<button class="item-translate-btn" title="Translate" data-text="${escapeHtml(cluster.primaryTitle)}"><span style="font-size:10px;opacity:0.5">A&#1488;</span></button>` : ''}
        </div>
        ${relatedAssetsHtml}
      </div>
    `;
